-- OLYMPUS DATABASE - ALL IN ONE
-- File SQL duy nhất cho cả project Supabase mới và database Olympus đang sử dụng.
-- Có thể chạy lại: không xoá lớp, học sinh, lịch, sổ chủ nhiệm hay cấu hình hiện có.
...
insert into classes (id, name, archived, sessions)
select seed.id, seed.name, seed.archived, seed.sessions
from (values
  ('c1', 'F12', false, array['S1', 'S2', 'C', '57', 'T']::text[]),
  ('c2', 'F13', false, array['S1', 'S2', 'C', '57', 'T']::text[]),
  ('c3', 'F14', false, array['S1', 'S2', 'C', '57', 'T']::text[])
) as seed(id, name, archived, sessions)
...
begin
  ...
  where key = any(clean_current)
    and (clean_name(value->>'location') <> '' or clean_name(value->>'note') <> ''
      or clean_name(value->>'startTime') <> '' or clean_name(value->>'teacherName') <> '');
...
-- Safe to run repeatedly as part of this all-in-one database file.
...
--    the earlier definitions, so keep this block after the earlier RPCs.
-- ---------------------------------------------------------------------------
...
-- Olympus schedule v2
-- Compact/detailed planner and course lesson counters.
...
-- Olympus schedule v3
-- Adds four independently saved lanes per class slot.
...
-- Stores the shared operating profile, presets,
...
-- Reusable templates for each class.
